...
namespace AC_facturero
{
    class CrearTicket
    {
        ...
        {
            linea.AppendLine("ARTICULO    |CANT|PRECIO|SUBTOTAL");
        }
        ...
        public void AgregaArticulo(string articulo, int cant, decimal precio, decimal importe)
        {
            ...
            if (cant.ToString().Length <= 5 && precio.ToString().Length <= 7 && importe.ToString().Length <= 8)
            {
                ...
                if (articulo.Length > 12)
                {
                    ...
                    for (int longitudTexto = articulo.Length; longitudTexto > 12; longitudTexto -= 12)
                    {
                        if (bandera == false)//si es false o la primera linea en recorrerer, continuar...
                        {
                            ...
                            linea.AppendLine(articulo.Substring(caracterActual, 12) + elemento);
                            bandera = true;//cambiamos su valor a verdadero
                        }
                        else
                            linea.AppendLine(articulo.Substring(caracterActual, 12));//Solo agrega el nombre del articulo

                        caracterActual += 12;//incrementa en 20 el valor de la variable caracterActual
                    }
